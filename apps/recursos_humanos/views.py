...
def personal_update(request, pk):
    try:
        ...
        # Amonestación: interpretar correctamente booleanos y strings que representen true/false
        ...
        # Guardar los valores en el propio modelo Personal
...
def generar_plantel(request):
    try:
        ...
        try:
            # Intentar usar MainPool unificado si existe
            from .models import MainPool
            nombres = list(MainPool.objects.filter(tipo='nombre').values_list('valor', flat=True)) or []
            apellidos = list(MainPool.objects.filter(tipo='apellido').values_list('valor', flat=True)) or []
            salarios_permitidos = [int(v) for v in list(MainPool.objects.filter(tipo='salario').values_list('valor', flat=True))] or [s for s in range(800, 3001, 250)]
        except Exception:
